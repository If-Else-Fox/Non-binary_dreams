...
03 июля 2025 года
	...
	Сегодня я попробую минимизировать залипание, для чего реализую две идеи:
	    * Самоосознание залипания (stick-awareness):
	            Если переменная слишком долго остаётся в +2 или –2, то модель
	            понимает, что застряла, и включает стратегию "отпуска". Для
	            этого я ввожу MAX_STICK = 15 (примерно 15 шагов подряд
	            к краю), и если переменная "залипает", то вес флага игнорируется,
	            и переменная постепенно возвращается к нулю.
	    * Импульс сброса
	            Если застряла вся система (например, все 4 переменные на краю), то
	            запускается мягкий глобальный сброс.
	
	    Я ввожу в программу дополнительные данные:
	        stick_count - считает, на сколько шагов переменная “залипла”
	        max_stick = 15 - порог, после которого начинается самоосвобождение
	        all_stuck - проверка, если все переменные в крайних значениях
	
	
	    Первая попытка решить эту задачу успехом не увенчалась: программа "Idea_07.py" показала
	...
	Случайно сгенерированные веса:
	  ...
	  B: {'b': 0.85, 'd': 0.33}
	  C: {'d': 0.6, 'b': 0.72}
	  D: {'c': 0.74, 'd': 0.21, 'b': 0.96}
	  Z: {'b': 0.82, 'a': 0.85, 'c': 0.37}
	
	| РЕЗУЛЬТАТ ЭКСПЕРИМЕНТА
	| Всего запусков цепочек: 1000000
	| Шагов в каждой цепочке: 100
	| Уникальных комбинаций: 865417
	| Число случаев, упершихся в предел [-2,2]: 707961
	| Доля уникальных комбинаций: 86.54%
	
	
	    Причина неправильной работы заключается в перегрузке линий b и d - почти все веса направлены на них:
	...
	b встречается в 5 флагах из 5, а d — в 3 из них. a и c используются мало, а a — вообще в одной строке.
	
	
	    Сушествует, как мне кажется, три варианта решения этой задачи:
	    * Равномерная генерация веса:
	        можно создать генератор весов, который гарантирует, что каждая переменная будет затрагиваться
	        с одинаковой частотой и иметь сбалансированные веса (например: 0.3–0.7)
	    * Активное отталкивание:
	        если переменная недавно была в краю — она становится менее чувствительной к весам, или получает
	        обратную силу.
	    * "Сон" нейрона:
	        если переменная слишком часто используется — она на шаг "засыпает": игнорирует сигнал
	        или пропускает изменение.
	
	
	    Я попробую пойти путем равномерной генерации веса и введу новые параметры
	для функции generate_random_weights():
	    * убеждаемсяся, что каждая переменная попадает хотя бы в 2 флага
	    * вес каждого ключа заклюяаем в разумных пределах, например от 0.3 до 0.9
	    * распределение делаем сбалансированным и контролируемым
	
	Обновленный generate_random_weights() выглядит следующим образом:
	...
	def generate_balanced_weights():
	    ...
	    # Каждый флаг будет затрагивать 2–4 переменные
	    for flag in flags:
	    ...
	    # Проверка: каждая переменная должна быть хотя бы в 2 флагах
	    ...
	    for var, count in variable_coverage.items():
	        if count < 2:
	            # Добавим эту переменную в случайные флаги, если она редкая
	            needed = 2 - count
	...
	Программа "Idea_08.py", в которой я ввел равномерную генерацию весов, показала следующие результаты:
	
	Случайно сгенерированные сбалансированные веса:
	  A: {'c': 0.71, 'd': 0.45, 'a': 0.72}
	  B: {'b': 0.81, 'a': 0.49, 'd': 0.83}
	  C: {'c': 0.56, 'd': 0.41, 'b': 0.59, 'a': 0.36}
	  D: {'d': 0.37, 'c': 0.5, 'b': 0.34, 'a': 0.69}
	  Z: {'c': 0.83, 'a': 0.81, 'b': 0.9, 'd': 0.31}
	
	| РЕЗУЛЬТАТ ЭКСПЕРИМЕНТА
	| Всего запусков цепочек: 1000000
	| Шагов в каждой цепочке: 100
	| Уникальных комбинаций: 742377
	| Число случаев, упершихся в предел [-2,2]: 474387
	| Доля уникальных комбинаций: 74.24%
	
	
	    Вывод:
	        * сбалансированные веса дали эффект: ни одна переменная не перегружена.
	          Все переменные (a, b, c, d) участвуют равномерно.
	        * достаточно высокая уникальность (74%) означает, что модель не стагнирует,
	          а исследует пространство состояний.
	
	    Недостатки модели:
	        * почти половина состояний всё ещё прилипают к краям (-2, 2).
	          Да, это естественно при агрессивных весах, даже сбалансированных, но это
	          мне не нравится. Вероятно, весовое воздействие всё ещё слишком велико,
	          и даже один сильный импульс v = ±2 толкает переменные к краям с огромной скоростью.
	
	    Стратегии дальнейшего улучшения работы модели:
	        * можно добавить “усталость” веса (адаптацию): если переменная часто изменяется,
	          то её вес временно падает - это смягчит накопления и создаст "жизненное сопротивление".
	        * можно ввести микро-ослабление: каждую итерацию все переменные понемногу стремятся к нулю,
	          например state[k] -= 0.01 * sign(state[k])
	        * можно снизить диапазон v: сделать v равномерным в [-1, 0, 1] — это уменьшит резкие скачки
	          и повысит разнообразие.
...
04 июля 2025 года
	...
	На неопределенный срок выпадаю из проработки данного проекта по техническим причинам.
